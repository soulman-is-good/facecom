    ...
    $('[facecom="crop"]').click(function(){
                ...
                $('<iframe name="fc-avatar-upload" id="fc-avatar-upload" style="position:absolute;left:-1000px;top:-1000px;visibility:hidden"></iframe>').load(function(){
                    ...
                    if(typeof json  == 'object' && json.status == 'OK'){
                        ...
                        if(href.indexOf('editBackground')!==-1)
                            return userBackgroundHandle.call(self,'/upload/photos/full/'+image,href);
                        var img = $('<img />');
                        img.load(imageSet).attr('src','/upload/photos/full/'+image);
                    }
                    else{
        ...
        tabs.append(
            $('<div />').attr({
                ...
                'display':'none'
            }).append($.facecomGallery({multiselect:false,path:'/upload/photos/',thumbsName:'small',rememberSelection:false,
                check:function(item,ui){
                    ...
                    }).attr('src',this.getPath()+item.filename+item.file_ext);
                ...
                select:function(item){
                        ...
                        if(href.indexOf('editBackground')!==-1){                            
                            userBackgroundHandle.call(this,'/upload/photos/' + item.filename,href);
                        }else{                            
                            $.post('/my/resize',{'image':'upload/photos/' + item.filename,'type':'avatar'},function(m){
                                if(m.status == 'OK'){
                                    ...
                                    $('<img />').load(imageSet).attr('src','/upload/photos/full/' + m.message);
    ...
    img.css('cursor','pointer').load(function(){
        ...
        var ok_button = $('<input />').attr({'type':'button','value':'Готово'}).css({'float':'right'}).click(function(){
            $.post(href,{
                ...
                if(m.status == 'OK'){
                    $('.userBG > img').attr('src','/upload/UserProfile/'+m.message+'?'+Math.random()*100);
                    rwnd.close();
