...
<div class="entry" style="position: relative; <?= $display ?>" entry_id="<?= $item->id ?>">
	...
	<div class="author">
		<? if ($item->parent_id != 0): ?>
			...
			?>
			<div>
				<img style="margin-top: 4px;" src="<?= Yii::app()->request->baseUrl ?>/static/css/publishedBy.gif" alt="" title="Автор оригинала" /> <a href="<?= Yii::app()->request->baseUrl ?>/id<?= $o_author->user_id ?>"><img style="margin-top: 4px;" src="<?= Yii::app()->request->baseUrl ?>/upload/UserProfile/32x32/<?= $o_author->avatar ?>" alt=""></a> <a href="<?= Yii::app()->request->baseUrl ?>/id<?= $o_author->user_id ?>"><strong><?= $o_author->first_name ?> <?= $o_author->second_name ?></strong></a>
			</div>
                        <?
                            endif;//$o_post == null
                        else: ?>
