 ...
 * @property User $user
 */
class Filemarks extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return Filemarks the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('file_id, lat, long', 'required'),
			array('status', 'numerical', 'integerOnly'=>true),
			array('file_id', 'length', 'max'=>50),
			...
			array('id, file_id, lat, long, zoom, status, created_at', 'safe', 'on'=>'search'),
		);
	}
