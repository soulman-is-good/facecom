...
//        Yii::app()->clientScript->registerPackage('main');
    ...
    public function actionGet() {
        ...
        $cri = new CDbCriteria(array(
            'select'=>"t.lat, t.long, t.zoom, CONCAT('/id$id/photos/show/',a.id,'#',p.id) as id, CONCAT(f.id,'.',f.extension) `filename`, f.id AS `file`",
        ...
        $placemarks = $model->getCommandBuilder()
               ...
               ->queryAll();
        $placemarks = array_map(create_function('&$item', 
                'if(empty($item["filename"]) || !is_file("upload/photos/".$item["filename"])) 
                    $item["filename"]="/images/avatar/default_profile.png";
                 else 
                    $item["filename"]="/images/80x80/".$item["file"];
                 return $item;
                 '),$placemarks);
