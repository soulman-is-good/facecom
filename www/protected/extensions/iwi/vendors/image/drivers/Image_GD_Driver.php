...
class Image_GD_Driver extends Image_Driver {
	...
	public function process($image, $actions, $dir, $file, $render = FALSE)
	{
		...
		{
			case IMAGETYPE_JPEG:
				$this->create = 'imagecreatefromjpeg';
			break;
			case IMAGETYPE_GIF:
				$this->create = 'imagecreatefromgif';
			break;
			case IMAGETYPE_PNG:
				$this->create = 'imagecreatefrompng';
			break;
		}
		...
		if (empty($this->create) OR ! function_exists($this->create))
			throw new CException('image type not allowed');

		// Make sure the image type is supported for saving
		if (empty($this->save) OR ! function_exists($this->save))
			throw new CException('image type not allowed');
                $create = $this->create;
                $save = $this->save;
		// Load the image
...
//            imagealphablending($copy, true);  
            if(file_exists($background)){
                ...
                imageellipse($res, $circle_x+1, $circle_y+1, 5, 5, imagecolorallocate($res, 255, 255, 255));
                imagecopymerge($res, $copy, 0, 0, 0, 0, $w, $h, 100);
                //imagecolortransparent($res, $mask_magicpink); 
                imagedestroy($this->tmp_image);
                $this->tmp_image = $res;
                imagedestroy($copy);
            }else{
                imagecolortransparent($copy, $mask_magicpink); 
                imagedestroy($this->tmp_image);
                $this->tmp_image = $copy;
            }
            $this->save = 'imagepng';
